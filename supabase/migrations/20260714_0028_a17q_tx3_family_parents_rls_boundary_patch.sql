-- A-17Q-TX3: family_parents RLS boundary patch.
-- Replaces only public.execute_admin_a17q_legacy_family_reconciliation after
-- the owner-authenticated non-dry-run attempt reached the first
-- public.family_parents UPDATE and PostgreSQL rejected the row under RLS.
-- Migration 0027 is already applied; this patch preserves the reviewed
-- signature, fixed search_path, grants, hashes, manifests and runtime caller,
-- and changes only the transaction-boundary RPC security mode.
