...
-- corrective replacement for the Gia Pha 4 official import transaction RPC, backed by:
...
--
-- This migration intentionally preserves the function signature, SECURITY
-- INVOKER mode, search_path, permission checks, ownership checks, locking,
-- audit, rollback, all-or-nothing transaction, idempotency, and execute
-- revokes from 0016. It only fixes the people.is_living derivation so JSON
-- null/missing/empty/invalid isLiving values fall back to deathDateText and
-- never produce SQL NULL for the NOT NULL people.is_living column.
...
comment on function public.a16p_tx_execute_giapha4_official_import(
  ...
  'A16BU corrective official import is_living null contract fix. NOT_APPLIED in repo phase; owner must manual apply and verify before any A-16R retry.';
