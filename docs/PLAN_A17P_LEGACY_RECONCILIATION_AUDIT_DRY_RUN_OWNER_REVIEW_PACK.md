...
## A-17P Manual Owner Approval Evidence
...
Owner-approved role confirmation group references:
...
The owner supplied person-level role confirmations in the production review
context. The repository records only sanitized group references and aggregate
counts here; production names are intentionally not committed. A-17P-R must
finalize an immutable owner decision pack from the owner-approved policy and
the production query output, then hash that pack before any execution phase.
...
## A-17P-R Immutable Owner Decision Pack
...
The finalized decision pack is immutable evidence and an authority candidate
for a later transaction-executor design phase. It does not authorize execution.
The pack contains UUIDs and safe refs needed for later design, but no
production names, private notes, contact data, account data or authentication
data. The excluded one-parent group and deleted-family advisory remain outside
the execution scope.
