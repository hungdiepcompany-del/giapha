...
- Existing backup-service only: top-level scaffold has no R2. Non-inherited `env.fixture-local` has R2 `remote:false` and both required secret names; this affects local dev only, and named-environment deploy remains forbidden/Owner-gated.
- AES-256-GCM/12-byte random IV, stable manifest AAD, plaintext/ciphertext/artifact SHA256, exact allowlist `CD811B...`, bounded request/object, unique artifact-hash key, put/get only, hashes/counts only. Shared-bucket, auth/scaffold, fixture, oversize, wrong-key/tamper/flags tests and all M2A/pipeline/17 legacy/types/typecheck/lint/diff checks PASS.
- Initial Reviewer FAIL P1/P2; fresh Owner GO; FIX-1; final `Terra/high` Reviewer PASS and `Luna/medium` Verifier PASS. Coder `Terra/high`, Explorer `Luna/medium` (cheapest-capable); initial corrections=2, FIX-1 corrections=1, fresh Coder executions=2 total. No main Worker/dependency/workflow mutation; backup service Wrangler changed only; no bucket/remote/deploy/prod/SQL/import/Auth-RLS/stage/commit/push. Temporary latest-types folder was cleaned by Controller.
...
- Next direction: one separate Owner gate for a clean main-based local checkpoint containing exactly 16 M2A implementation paths plus these 3 closeout docs, only the verified candidate, preserving inherited dirt. Exact hashes are not yet claimed; no push/deploy/bucket/production.
...
- Evidence artifacts:
  `EVIDENCE_FILE=docs/PLAN_A17Q_TX1R_LEGACY_FAMILY_RECONCILIATION_EXECUTOR_MANUAL_APPLY_VERIFICATION.md`;
  `CHECKER=scripts/check-a17q-tx1r-legacy-family-reconciliation-executor-manual-apply-verification.cjs`;
  `PACKAGE_SCRIPT=check:a17q-tx1r-legacy-family-reconciliation-executor-manual-apply-verification`.
- Function/verifier contract:
  `FUNCTION_EXISTS=YES`;
...
## 2026-07-13 - A-17Q-TX1-FIX3-REVIEW - Final migration candidate source approved
...
- Reviewed authority:
  ...
  `MIRROR_MATCH=YES`.
- Evidence artifacts:
  `REVIEW_EVIDENCE_FILE=docs/PLAN_A17Q_TX1_FIX3_OWNER_REVIEW_FINAL_MIGRATION_CANDIDATE.md`;
  `FIX3_OWNER_REVIEW_CHECKER=scripts/check-a17q-tx1-fix3-owner-review.cjs`;
  `PACKAGE_SCRIPT=check:a17q-tx1-fix3-owner-review`.
- Review results:
...
## 2026-07-13 - A-17Q-TX1-FIX3 - Final integrity contract ready for owner review
...
- Artifacts:
  ...
  `PACKAGE_SCRIPT=check:a17q-tx1-fix3-final-integrity-contract`.
- Hashes:
...
## 2026-07-13 - A-17Q-TX1-FIX2-REVIEW - Exact post-state review blocked
...
- Positive review notes:
  immutable manifest, SECURITY INVOKER/search_path/grants, pre-mutation
  snapshots and audit, exact child mapping, parent/role, family void and merge
  target post-state checks passed source review.
- Blockers:
...
- Positive review notes:
  idempotency replay/conflict/recovery branches, dry-run before batch insert,
  pre-mutation audit order, mutation CTE row-count enforcement, post-audit,
  security/grants and active runtime caller count passed source review.
- Blockers:
...
## 2026-07-13 - A-17P manual owner approval evidence recorded
...
- Expected execution pack counts:
...
- Boundary:
  ...
  `PUSH=NO`.
- Note:
  Person-level production names from owner review were intentionally not
  committed; use the owner-approved production result and policy in A-17P-R.
- Next action:
  `NEXT_ACTION=A17P_R_FINALIZE_IMMUTABLE_OWNER_DECISION_PACK`.

## 2026-07-13 - A-17P-FIX3 - Parent role gender review evidence ready
...
## 2026-07-13 - A-17P-FIX2 - Owner-facing legacy family review query ready
...
## 2026-07-13 - A-17P-FIX1 - Legacy reconciliation audit corrected before owner review
...
## 2026-07-13 - A-17P - Legacy reconciliation review pack ready
...
## 2026-07-13 - A-17O-DR - Grouped importer deploy smoke evidence recorded
...
## 2026-07-13 - A-17O-R - Grouped importer runtime source integrated
...
## 2026-07-13 - A-17O-TX1R - Grouped executor manual apply verification recorded
...
- Finding:
  ...
  the current `public.a16p_tx_execute_giapha4_official_import` SQL creates one
  `family_id` per child in `clear_children`, so the app cannot activate a
  one-family-many-children plan safely yet.
- Dormant foundation:
...
## 2026-07-12 - A-17N-DR - Deploy and production no-mutation smoke evidence recorded
...
- Validation:
  A-17N-TX2F, A-17N-TX1, A-17N, A-17M, A-17A through A-17I, A-16R
  post-import, relationship/tree/public privacy, env-safe, migrations,
  typecheck and lint all PASS. Direct checkout build hit the known Windows
  `.next` ACL lock before compile; clean temp-copy `npm.cmd run build` under
  `.tmp\a17n-tx2r-build-20260712` PASS.
- Next action:
...
- Migration candidate:
  ...
  db/Supabase mirrors are intended to remain byte-identical.
- Verifier:
  `db/checks/20260712_check_a17n_tx1_admin_canonical_family_transaction_executor.sql`;
  `POST_APPLY_VERIFIER_CREATED=YES`;
  `POST_APPLY_VERIFIER_EXECUTED=NO`.
...
- Safety:
  ...
  `PACKAGE_DEPENDENCY_INSTALLED=NO`.
- Next action:
  `OWNER_REVIEW_A17M_THEN_START_SEPARATE_A17N_ADMIN_PARENT_CHILD_WRITE_PATH_INTEGRATION`.
...
- Reconciliation blockers preserved:
...
- Post-apply verifier:
  `POST_APPLY_VERIFIER_CREATED=YES`;
  `POST_APPLY_VERIFIER_EXECUTED=NO`;
  run only in the separate owner-approved A-17I/post-apply phase after
  migration 0023 is manually applied.
- Owner review marker required:
  `APPROVE_A17H_CANONICAL_FAMILY_SCHEMA_CANDIDATE`.
- Next action:
  `OWNER_REVIEW_A17H_THEN_RUN_SEPARATE_A17SQL_H_MANUAL_APPLY_PHASE`.
...
- Dry-run result:
  ...
  all preservation invariants PASS.
- Rollback:
...
- Production reconciliation remains blocked:
  owner review groups exist, invalid person references exist, backup evidence is
  absent, schema foundation is missing, and write-path repair is not deployed.
- Safety:
  `SCHEMA_CREATED=NO`;
  `MIGRATION_CREATED=NO`;
  `MIGRATION_APPLIED=NO`;
...
- Supabase SELECT-only verification after owner acceptance:
  ...
  `A16BT_A16BR_REVISIONS_POLICY_PRESERVED=YES`;
  `A16BT_POLICY_COUNT=4`.
- Safety:
  no mutation SQL in this acceptance update, no production genealogy rows
  queried, no official-import RPC, no `POST /official-import`, no deploy and
  no push.
- Next action:
  `A16BT_NEXT_ACTION=COMPLETE_NO_IMPORT_RETRY_NO_DEPLOY`.

## 2026-07-11 - A-16BT - Owner-approved verify continuation
...
- Migration hashes:
  `A16BT_MIGRATION_0020_HASH_MATCH=YES_530129F27EAD748641C71D2C26718043D0B51639FC6104EFFC4B9D222550C0FC`;
  `A16BT_MIGRATION_0021_HASH_MATCH=YES_A7277E8A682610447BEC8142564C1A94B5FDE1AB4726C76D7DDF8205486B5D2C`.
- Supabase SELECT-only verification:
  `A16BT_BROAD_ANON_SELECT_REVOKED=YES`;
  `A16BT_SAFE_COLUMN_GRANTS_VERIFIED=YES`;
  `A16BT_PRIVATE_COLUMN_EXPOSURE=NO`;
  `A16BT_ANON_RLS_VERIFIED=YES`;
  ...
  `A16BT_A16BR_REVISIONS_POLICY_PRESERVED=YES`.
- Localhost smoke:
  `LOCALHOST_HOME_SMOKE=PASS_HTTP_200_NO_PRIVATE_FIELD_PAYLOAD_TOKENS`;
  `LOCALHOST_TREE_SMOKE=PASS_HTTP_200_NO_PRIVATE_FIELD_PAYLOAD_TOKENS`;
  `LOCALHOST_PERSON_SMOKE=SAFE_SKIP_NO_PUBLIC_PERSON_LINK_IN_RENDERED_PUBLIC_ROUTES`;
  `LOCALHOST_PUBLIC_PREVIEW_SMOKE=PASS_HTTP_200_NO_PRIVATE_FIELD_PAYLOAD_TOKENS`.
- Validation:
  A16BT, A16BR, A16BQ, env-safe, migrations, typecheck, lint and build all
  passed. Build required safe workspace-local `.next` cleanup after Windows
  `EPERM`.
- Safety:
  no production genealogy rows queried, no official-import RPC, no
  `POST /official-import`, no deploy and no push.
- Next owner action:
  `A16BT_NEXT_ACTION=OWNER_RECONCILE_OR_ACCEPT_MANUAL_SQL_MIGRATION_HISTORY_GAP_BEFORE_MARKING_PASS_NO_IMPORT_RETRY`.
...
- Migration 0021:
  ...
  `A16BT_MIRROR_MATCH=YES_DB_AND_SUPABASE_MIGRATION_0021_BYTE_IDENTICAL`.
- Verification SQL:
...
- Safety:
  no SQL run, no migration apply, no production genealogy-row query, no
  Supabase db push/repair/seed, no POST `/official-import`, no import RPC, no
  deploy, and no push.
- Next owner action:
  `A16BT_NEXT_OWNER_ACTION=REVIEW_0021_THEN_SEPARATE_OWNER_APPLY_VERIFY_AND_LOCALHOST_PUBLIC_SMOKE_NO_IMPORT_RETRY`.
...
- Next owner action:
  run the A-16BQ SELECT-only metadata runbook in a separate owner verification
  phase and record every boolean/count before any later retry consideration.
- A-16R retry:
  `A16R_IMPORT_RETRY_NEXT=NO`.
...
- Production blocker after A-16BK:
...
- Owner production evidence before this phase:
  POST returned 409 `BLOCKED`, `canRunOfficialImport=true`,
  `A16AH_BLOCKED_TRANSACTION_RPC_FAILED`, `SESSION_NOT_FOUND_OR_NOT_OWNED`,
  imported people count `0`, and the transaction helper was reached once.
...
- Diagnostic response boundary:
  authenticated strict-permission GET only; sanitized booleans and
  classifications only; no IDs, names, tokens, cookies, raw JSON or genealogy
  rows; no import transaction RPC.
...
- Safety:
  no POST `/official-import`, no A-16R retry, no import transaction RPC, no
  SQL/DB/session/auth/permission/genealogy mutation, no deploy, no raw/private
  data print or commit in A-16BH.
- Next action:
...
- Write manifest:
  `A16BB_WRITE_MANIFEST_STATUS_COUNTS=draft:1` and
  `A16BB_OWNER_APPROVED_WRITE_MANIFEST_COUNT=0`.
- Source update:
  runtime service, GET preflight, and owner UI now use
  `buildOfficialImportSessionStateGate`; obsolete runtime `staged` check is
  gone.
- A-16R import retry remains:
  `A16R_IMPORT_RETRY_NEXT=NO`.
- Safety:
...
- Canonical official-import pre-write states:
...
- Read-only smoke result:
  ...
  `Báº¡n chÆ°a cÃ³ quyá»n imports.create.`.
- A-16R/env flag state:
  import panel not rendered, A-16R block not visible, audited session not
  visible, runtime/session markers not visible, runtime candidate env and
  execution branch env not proven, final confirmation checkbox not visible and
  official import button state `NOT_RENDERED`.
- Blocker:
...
- Production UI path checked:
  `/admin/exports/import`.
- Same-run browser context:
  redacted authenticated non-owner context, role `NO_ROLE`, visible permission
  count `0`, and missing `imports.create`.
- Blocker:
  `A16AM_BLOCKER=AUTHENTICATED_OWNER_ADMIN_IMPORT_CONTEXT_NOT_AVAILABLE_OR_PERMISSION_INSUFFICIENT`.
...
- A-16R import retry remains:
  `A16R_IMPORT_RETRY_NEXT=NO`.
- Safety:
  no POST `/official-import`, no direct/manual RPC, no SQL/DB mutation outside
  approved import path, no migration repair, seed, db push, deploy, Wrangler
  deploy, auth/user/role/permission/membership mutation, raw JSON/private data
  print, raw JSON commit, `wrangler.toml` edit or `app/layout.tsx` edit.
- Next action:
  Log in with a real owner/admin import context that has the strict permission
  set, then start a separate execution phase that rechecks same-run gates before
  any single approved POST. Do not retry from this blocked evidence.
...
- Do not call POST `/official-import`, retry A-16R import, call direct RPC
  ...
  Wrangler deploy, run local Windows deploy, edit `wrangler.toml`, edit
  `app/layout.tsx`, commit raw JSON, print private JSON contents, or print a
  final import command from A-16AD.
...
Project guard V2 repository-local isolation:
