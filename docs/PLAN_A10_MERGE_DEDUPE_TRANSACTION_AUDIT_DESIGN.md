...
Review xác nhận A-10 đã mô tả đầy đủ candidate detection chỉ mang tính gợi ý,
transaction all-or-nothing, version check, conflict review, graph validation,
audit impact, rollback manifest, permission gate và UI tiếng Việt. Thiết kế đủ
điều kiện để owner cân nhắc cấp marker
`APPROVE_A10_MERGE_DEDUPE_RUNTIME_DESIGN` nhằm mở A-11 schema candidate.

Marker status: `NOT_GRANTED_BY_THIS_REVIEW`. Kết luận review không tự đăng ký
permission, không mở schema/runtime và không thay thế hành động approval rõ
ràng của owner. A-11 vẫn chỉ là schema candidate/docs/checker; không migration,
DB apply hoặc runtime merge nếu marker được cấp.
