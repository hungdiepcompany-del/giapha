...
- Preserved inherited dirty Auth/OAuth, A17/A17P tree print, Supabase, permission, `_guard`, `.next_locked_*`, temp logs and artifact files.
- Current-session continuity:
  - Upload staging response now exposes top-level `sessionId`, `manifestId`, `status`, staging row/person/relationship counts and warning count.
  - Upload success navigates to `/admin/exports/import?sessionId=<UUID>`.
  - The import page reads only an explicit valid `sessionId` from the URL; missing URL session shows a no-session state and invalid UUIDs fail closed.
  - No latest-session fallback was added.
- A-16G/readiness workflow:
  - Manifest panel is now passed the current explicit session and derives workflow gates from that session.
  - Historical A-16R session `2af4bfb6-a20e-453e-9804-1b8c0afbdd68` remains audit evidence only.
  - Legacy Excel/family JSON tools are separated from the primary workflow area.
- Validation/warning review:
  - Validation summaries expose `sessionId`, `manifestId`, `stagingVersion` and separated blocker count.
  - Added warning review grouping and acknowledgement source bound to session, manifest, staging version/hash, warning group, actor and policy marker.
  - The warning acknowledgement route was not executed by Codex.
- Dry-run/approval/readiness:
  - Dry-run approval no longer compares current work against the historical A-16R session.
  - Owner approval markers/routes are dynamically derived from the current session.
  - Official import confirmation markers are dynamically derived from the current session, but the UI remains disabled in this phase.
- Safety evidence:
...
- Existing legacy A-16 checkers `check:a16i-upload-parse-giapha4-manifest-staging`, `check:a16g-import-session-read-manifest-runtime`, `check:a16j-manifest-staging-review-validation-warnings`, `check:a16l-dry-run-mapping-preview` and `check:a16i5-import-review-pack-official-import-gate` FAIL only on frozen single-phase allowlists: inherited A17/Auth/guard/artifact dirt and the intentional new warning acknowledgement route are outside those old scopes. Do not revert unrelated files to satisfy those obsolete scope checks.
- `NEXT_ALLOWED_PHASE=A-16R3_OWNER_GATED_OFFICIAL_IMPORT_EXECUTION`.
...
Phase: A-17Q-TX1R - Record Manual Apply and SELECT-Only Verification Evidence
...
Owner-provided SELECT-only verifier evidence:
...
- Added `docs/PLAN_A17Q_TX1R_LEGACY_FAMILY_RECONCILIATION_EXECUTOR_MANUAL_APPLY_VERIFICATION.md`.
- Added checker `scripts/check-a17q-tx1r-legacy-family-reconciliation-executor-manual-apply-verification.cjs`.
- Added package script `check:a17q-tx1r-legacy-family-reconciliation-executor-manual-apply-verification`.
...
## 2026-07-13 - A-17Q-TX1-FIX3 owner review approves final migration candidate

Phase: A-17Q-TX1-FIX3-REVIEW - Final Owner Review Before Manual Apply
...
- Added `docs/PLAN_A17Q_TX1_FIX3_OWNER_REVIEW_FINAL_MIGRATION_CANDIDATE.md`.
- Added static checker `scripts/check-a17q-tx1-fix3-owner-review.cjs`.
- Added package script `check:a17q-tx1-fix3-owner-review`.
- Preserved migration 0026 unchanged during review.
...
- `A17Q_TX1_REVIEW_STATUS=BLOCKED_SOURCE_CORRECTION_REQUIRED` superseded by source correction
...
- Corrected existing not-applied migration 0026 in both `db/migrations` and `supabase/migrations`.
- Added narrow A-17Q revision insert policy for executor audit rows.
- Hardened dry-run/idempotency behavior so dry-run does not consume a batch and replay only returns stored completed success.
- Moved running batch insert after full precondition validation.
- Added pre-mutation audit before genealogy mutation and post-mutation audit after real post-state validation.
- Captured mutation CTE row counts and fail-closed on mismatches.
- Added post-state count/hash/graph validation before storing completed result.
- Extended SELECT-only verifier and base checker.
- Added checker `scripts/check-a17q-tx1-fix1-hardened-reconciliation-executor.cjs`.
- Added package script `check:a17q-tx1-fix1-hardened-reconciliation-executor`.
...
## 2026-07-13 - A-17Q-TX1 legacy family reconciliation transaction executor candidate

Phase: A-17Q-TX1 - Design Legacy Family Reconciliation Transaction Executor Candidate
...
- Created mirrored not-applied migration 0026 with an embedded decision-pack manifest.
- Created SELECT-only post-apply verifier candidate for a future owner phase.
- Added checker `scripts/check-a17q-tx1-legacy-family-reconciliation-transaction-executor-candidate.cjs`.
- Added package script `check:a17q-tx1-legacy-family-reconciliation-transaction-executor-candidate`.
- Updated A-17Q-TX1 plan, index, decision log and handoff.
...
## 2026-07-13 - A-17P-R immutable owner decision pack finalized

Phase: A-17P-R - Finalize Immutable Owner Decision Pack
...
- `DECISION_PACK_SHA256_FILE=docs/evidence/A17P_OWNER_DECISION_PACK.sha256`
- `SCHEMA_VERSION=a17p-owner-decision-pack:v1`
- `OWNER_APPROVAL_EVIDENCE_COMMIT=1f30228`
- `FIX3_SOURCE_COMMIT=19c6cb0`
- `APPROVED_GROUP_COUNT=21`
- `APPROVED_CANDIDATE_FAMILY_COUNT=57`
...
- `EXCLUDED_SCOPE_OVERLAP_COUNT=0`
- `DELETED_FAMILY_SCOPE_OVERLAP_COUNT=0`
...
- Created immutable decision pack JSON and matching SHA-256 file.
- Added checker `scripts/check-a17p-r-immutable-owner-decision-pack.cjs`.
- Added package script `check:a17p-r-immutable-owner-decision-pack`.
- Updated A-17P plan, owner review template, index, decision log and handoff.
...
- `NEXT_ACTION=A17Q_TX1_DESIGN_LEGACY_FAMILY_RECONCILIATION_TRANSACTION_EXECUTOR_CANDIDATE`

## 2026-07-13 - A-17P manual owner approval evidence

Phase: A-17P manual owner review result record
...
- Recorded owner manual approval evidence in the A-17P plan, index, decision log and handoff.
- Added checker `scripts/check-a17p-manual-owner-approval-evidence.cjs`.
- Added package script `check:a17p-manual-owner-approval-evidence`.
- Kept person-level production names out of committed docs.
...
- Added SELECT-only owner-facing review SQL `db/checks/20260713_check_a17p_owner_facing_legacy_family_review.sql`.
- Added checker `scripts/check-a17p-fix2-owner-facing-legacy-family-review-query.cjs`.
- Added package script `check:a17p-fix2-owner-facing-legacy-family-review-query`.
- Updated the A-17P owner review template with parent/child name and survivor review fields.
- Updated index, decision log and handoff.
...
- Added SELECT-only owner audit SQL `db/checks/20260713_check_a17p_legacy_family_reconciliation_audit.sql`.
- Added pure dry-run planner `scripts/a17p-legacy-reconciliation-planner.cjs`.
- Added checker `scripts/check-a17p-legacy-reconciliation-audit-dry-run-owner-review-pack.cjs`.
- Added owner review pack template `docs/templates/A17P_LEGACY_RECONCILIATION_OWNER_REVIEW_PACK_TEMPLATE.md`.
- Added A-17P plan documentation and synced A-17 audit/readiness docs.
...
Phase: A-17O-DR - Record Grouped Importer Deploy and Production No-Import-Mutation Smoke Evidence
...
- Added `docs/PLAN_A17O_DR_GROUPED_IMPORTER_DEPLOY_NO_IMPORT_MUTATION_SMOKE_EVIDENCE.md`.
- Added checker `scripts/check-a17o-dr-grouped-importer-deploy-no-import-mutation-smoke-evidence.cjs`.
- Updated A-17O/A-17O-R docs, index, decision log and handoff to record deploy/no-import-mutation evidence.
- Updated related checker allowlists for the documentation-only evidence phase.
...
A-17M - dormant canonical family domain service foundation

### Viec da lam

- Verified preflight: branch `main`, worktree clean, A-17I commit present,
  A-17H migration mirrors present, and A-17I post-apply evidence recorded.
- Added server-only canonical family domain foundation files under
  `lib/family/`.
- Added deterministic identity builder, parent-set normalization,
  repository-backed lookup contract, reuse/create decision logic, mutation-plan
  contract and no-PII diagnostic/error codes.
- Added checker-backed pure fixtures for required identity, decision and
  mutation-safety contracts.
- Did not modify admin add-parent/add-child, add-spouse, importer, public tree,
  graph builder, ELK layout, tree editor or tree viewer runtime paths.
- Did not create migration, execute SQL, mutate production data, backfill
  canonical keys, run reconciliation, call import RPC, deploy, push or install
  dependencies.
...
A-17H - not-applied canonical family schema foundation candidate

### Viec da lam

- Added additive migration candidate
  `db/migrations/20260712_0023_a17h_canonical_family_schema_foundation_candidate.sql`.
- Mirrored the byte-identical migration under
  `supabase/migrations/20260712_0023_a17h_canonical_family_schema_foundation_candidate.sql`.
- Added future post-apply SELECT-only verifier
  `db/checks/20260712_check_a17i_canonical_family_schema_post_apply.sql`.
- Added static checker
  `scripts/check-a17h-canonical-family-schema-foundation-candidate.cjs`
  and package script
  `check:a17h-canonical-family-schema-foundation-candidate`.
- Preserved prior migration 0022 hash and did not edit applied migrations.
- Did not execute SQL, did not apply migration 0023, did not update or backfill
  existing genealogy rows, did not reconcile families, did not create an
  execution RPC, did not call import RPC, did not deploy and did not push.
...
- Migration 0020:
  old full-anon-revoke SHA
  `A16BR_MIGRATION_0020_SUPERSEDED_SHA256=0A7F69196C97071C7304E4D0CE28DA1C134E95AF3DEFA00C8958FC7971591CF0`
  is superseded and must never be applied; corrected SHA
  `A16BR_MIGRATION_0020_SHA256=530129F27EAD748641C71D2C26718043D0B51639FC6104EFFC4B9D222550C0FC`
  preserves public core anon SELECT while revoking anon/PUBLIC mutation grants
  on `people`, `families`, `family_parents`, and `family_children`.
...
- Safety:
  ...
  `/official-import`, no import RPC, no production genealogy-row query, no
  deploy, no push, and `A16R_IMPORT_RETRY_NEXT=NO`.

## 2026-07-11 - A-16BR - Revisions INSERT RLS contract and anon grant cleanup candidate
...
- Safety:
  no SQL run, no Supabase db push/repair/seed, no POST `/official-import`,
  no import RPC, no session/genealogy mutation, no deploy, and no push.
...
- Audit/idempotency result:
  ...
  `A16BJ_ROLLBACK_MANIFEST_EXISTS=NO`, and
...
- Safety:
  no POST `/official-import`, no import RPC, no SQL/DB/session/auth/
  permission/genealogy mutation, no deploy, no raw/private data printed or
  committed.
- Next action:
...
- A-16R import retry remains:
  `A16R_IMPORT_RETRY_NEXT=NO`.
- Safety:
  no POST `/official-import` in this phase, no A-16R retry, no import RPC, no
  direct/manual RPC, no SQL/DB mutation, no session-state change, no deploy, no
  auth/role/permission/membership/genealogy mutation, no raw/private data print
  or commit.
- Next action:
...
- Result:
  admin shell loaded, but the active context showed visible permission count
  `0` and sanitized UI reason `Bạn chưa có quyền imports.create.`.
- A-16AR gate visibility:
  import panel was not rendered, so audited session, runtime/session markers,
  final confirmation checkbox and official import button were not visible.
- Blocker:
...
- Next action:
  owner must reopen the production admin import route with a true owner/admin
  import context and rerun read-only UI smoke. Do not submit import.
...
- Diagnosis:
  owner/admin permission and audited-session blockers are no longer the active
  source blocker; the source still keeps A-16R fail-closed because the UI
  button is hard-disabled, the GET preflight gate always returns
  `canOpenOfficialImport=false` and `officialImportEnabled=false`, review-pack
  official-import flags remain false by design, and the UI does not submit the
  required same-run confirmation body to the POST route.
- Classification:
  `A16AQ_DISABLED_CLASSIFICATION=SOURCE_UI_PREFLIGHT_REVIEW_PACK_STILL_FAIL_CLOSED_NO_POST_PLUMBING`.
- Browser GET blocker:
...
- Next action:
  ...
  that submits the required confirmation body only after all gates pass.
...
- Same-run owner/admin context:
  `A16AP_OWNER_PERMISSION_CONTEXT=PASS_SANITIZED_OWNER_ADMIN_CONTEXT`; role
  `OWNER`, permission count `25`, `imports.create` present,
  `permissions.manage` present, strict permission missing none.
- Production UI result:
  audited session, 102 proposed people, 134 proposed relationships and 94
  warnings were visible, but the A-16R official import button remained
  disabled.
- Blocker:
  `A16AP_BLOCKER=OFFICIAL_IMPORT_UI_RUNTIME_GATE_REMAINS_LOCKED_BUTTON_DISABLED_SAME_RUN_ROUTE_NOT_EXECUTION_CAPABLE`.
...
- Next action:
  diagnose why the owner markers and owner/admin permission context still do
  not enable the same-run official import route/button.
...
- Boundaries preserved: no POST `/official-import`, no direct RPC official
  import, no A-16R retry, no SQL/DB/auth/permission/genealogy mutation, no
  deploy, no Wrangler deploy, no raw JSON/private data print, and no raw JSON
  commit.
...
- Dry-run gate can open for the audited session only:
  `A16K_DRY_RUN_GATE_CAN_OPEN_FOR_AUDITED_SESSION=YES`.
- Other sessions remain blocked:
  `A16K_DRY_RUN_GATE_BLOCKS_OTHER_SESSIONS=YES`.
- Official import remains locked: `officialImportOpen=false`,
  `canRunOfficialImport=false`, `canOpenOfficialImport=false`,
  `officialImportEnabled=false` and the official import button remains
  disabled.
- Dry-run remains non-writing: `dbWrite=false`, `peopleWrite=false`,
  `relationshipWrite=false`, `treeLayoutWrite=false` and
  `revisionWrite=false`.
- Hydration advisory recorded as
  `LOCAL_HYDRATION_ADVISORY_LIKELY_BROWSER_EXTENSION_INJECTION`; `app/layout.tsx`
  was not changed and no `crxlauncher` attribute was added.
- A-16R import retry remains:
  `A16R_IMPORT_RETRY_NEXT=NO`.
- Boundaries preserved: no deploy, no Windows-local deploy, no POST
  `/official-import`, no confirm click, no direct RPC, no real genealogy write,
  no SQL, no DB push, no migration repair, no seed, no
  permission/role/auth/user/membership mutation and no `wrangler.toml` change.
...
- Observed production UI marker from owner evidence:
  `APPROVE_A16R_RUN_OFFICIAL_IMPORT_FOR_SESSION_ae7a5fe3-6a29-4f60-85f7-76108ed02565`.
- Previously tracked A-16R session marker:
  `APPROVE_A16R_RUN_OFFICIAL_IMPORT_FOR_SESSION_2af4bfb6-a20e-453e-9804-1b8c0afbdd68`.
- Source finding: the UI marker is rendered from `session.id`; the import page
  selects `listImportSessions().sessions[0]`, and `listImportSessions()` orders
  sessions by `created_at desc`.
- Session `2af4bfb6-a20e-453e-9804-1b8c0afbdd68` remains the only session with
  prior complete counts/readiness evidence: people `102`, relationships `134`,
  validation errors `0`, dry-run blockers `0`, duplicate unresolved `0`,
  needs_review `0`, create_new `8`.
- Session `ae7a5fe3-6a29-4f60-85f7-76108ed02565` has observed UI marker
  evidence only; counts, source identity, readiness, rollback/audit evidence,
  already-imported state and eligibility remain `UNKNOWN`.
- A-16R import retry remains:
  `A16R_IMPORT_RETRY_NEXT=NO`.
- Next allowed action:
...
- Boundaries preserved: no deploy, no POST `/official-import`, no confirm
  click, no direct RPC, no real genealogy write, no SQL, no DB push, no
  migration repair, no seed, no permission/role/auth/user/membership mutation,
  no direct production DB query and no `wrangler.toml` change.
...
- UI copy now separates `Cổng kiểm tra thử / dry-run A-16K` from
  `Cổng nhập chính thức A-16R`.
- The A-16K block now says dry-run only opens preview/mapping review and does
  not authorize official import execution or write real genealogy data.
- The A-16R block now says `Trạng thái hiện tại: nhập chính thức vẫn khóa`,
  displays session marker
  `APPROVE_A16R_RUN_OFFICIAL_IMPORT_FOR_SESSION_2af4bfb6-a20e-453e-9804-1b8c0afbdd68`
  and keeps runtime marker
  `APPROVE_A16R_RUNTIME_EXECUTION_AFTER_A16V_VERIFY` separate.
- Official import button remains disabled:
  `A16R_UI_COPY_REFRESH_OFFICIAL_IMPORT_BUTTON=DISABLED`.
- Source remains fail-closed:
  `A16R_UI_COPY_REFRESH_CAN_RUN_OFFICIAL_IMPORT=false` and
...
- Repo-local Windows build/deploy remains unsafe while the `.next` ACL lock
  and prior OpenNext Windows warning remain unresolved:
  `REPO_LOCAL_WINDOWS_NEXT_ACL_CAVEAT=EPERM_UNLINK_DOT_NEXT_BUILD_56416d4ae4ce586f_JS`.
- Candidate conclusion: keep `wrangler.toml`, `open-next.config.ts`,
  `next.config.ts`, deploy scripts and workflows unchanged; add only the
  checker package script for this docs/checker phase.
- A-16R import retry remains blocked:
  `A16R_IMPORT_RETRY_NEXT=NO`.
- Safety boundary: `A16R_OPENNEXT_CLOUDFLARE_DEPLOY_BUNDLE_DEPLOY_RUN=NO`,
...
- The session-specific run marker remains separate and is not enough:
  `APPROVE_A16R_RUN_OFFICIAL_IMPORT_FOR_SESSION_2af4bfb6-a20e-453e-9804-1b8c0afbdd68`.
...
- Reason: the latest bundle prompt still did not include actual verification SQL
  output after owner/manual apply. It contained only placeholder text for pasting
  the real SQL verification result, so A-16T PASS cannot be recorded.
...
- This update did not run SQL, did not DB push, did not run migration repair,
  did not seed, did not call RPC, did not call POST official import, did not
  write people/relationships/families/layout/tree/revision/profile data, did
  not deploy and did not push.
