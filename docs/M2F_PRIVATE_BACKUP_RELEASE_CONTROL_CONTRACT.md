...
Later `versions upload` is separately zero-traffic: `wrangler deployments status --json` must prove equal current routing before and after, and the exact tag/version must exist. Main upload uses `--keep-vars` without a backup secrets file. It also passes the four approved non-secret runtime values explicitly with `--var`: Supabase URL, Supabase publishable/anon contract value, and both A16 flags. This makes an intentional GitHub-variable change override the prior Worker version instead of being silently shadowed by `--keep-vars`. `NEXT_PUBLIC_APP_URL` remains build-only, and `SUPABASE_SERVICE_ROLE_KEY` remains an inherited encrypted secret rather than a plain variable. Wrangler 4.100.0 documents that secrets are never deleted by deployments, while `--keep-vars` preserves all other dashboard-managed non-secret variables. Promotion and rollback first prove the exact target version exists from `versions list`, verify the declared current deployment/version from `deployments status`, and then use `wrangler versions deploy <id>@100% --yes` followed by a current 100%-route proof. `deployments list` is history only and must never authorize traffic or satisfy a current-state precondition/postcheck.
