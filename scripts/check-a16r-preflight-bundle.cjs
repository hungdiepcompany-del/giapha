...
const doc = readFile(preflightDocPath);
...
for (const token of [
  ...
  requireIncludes(doc, token, `preflight doc token ${token}`);
}

if (
  packageJson?.scripts?.["check:a16r-preflight-bundle"] !==
  "node scripts/check-a16r-preflight-bundle.cjs"
) {
  failures.push("missing package script check:a16r-preflight-bundle");
}

for (const [content, token, label] of [
  [index, "PLAN_A16R_PREFLIGHT_BUNDLE.md", "index preflight entry"],
  [
    workLog,
    "A16R_PREFLIGHT_BUNDLE_STATUS=PASS_PREFLIGHT_READY_APPROVAL_REQUIRED",
    "work log preflight status",
  ],
...
for (const artifact of [
  ...
  if (!fs.existsSync(path.join(root, artifact))) {
    failures.push(`missing rollback/audit artifact ${artifact}`);
  }
}
...
if (failures.length > 0) {
  console.error("A-16R-PREFLIGHT-BUNDLE check failed:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log("A-16R-PREFLIGHT-BUNDLE check passed.");
