...
for (const token of [
  ...
  requireIncludes(runDoc, token, `run doc token ${token}`);
}

requireIncludes(postDoc, "A16R_RUN_STATUS=BLOCKED_REAL_TRANSACTION_EXECUTION_BRANCH_NOT_READY", "post doc blocked status");

if (
  packageJson?.scripts?.["check:a16r-run-official-import"] !==
  "node scripts/check-a16r-run-official-import.cjs"
) {
  failures.push("missing package script check:a16r-run-official-import");
}

for (const [content, token, label] of [
  [index, "PLAN_A16R_RUN_OFFICIAL_IMPORT.md", "index run import entry"],
  [workLog, "A16R_RUN_STATUS=BLOCKED_REAL_TRANSACTION_EXECUTION_BRANCH_NOT_READY", "work log run status"],
  [decisionLog, "Decision 233", "decision log A-16R run entry"],
  [handoff, "A16R_RUN_STATUS=BLOCKED_REAL_TRANSACTION_EXECUTION_BRANCH_NOT_READY", "handoff run status"],
]) {
...
for (const token of [
  ...
  requireIncludes(service, token, `service fail-closed token ${token}`);
}
...
for (const token of [
  ...
  requireIncludes(gate, token, `gate locked token ${token}`);
}
...
for (const token of [
  ...
  "No rollback manifest is persisted in A-16P-TX",
]) {
  requireIncludes(sql, token, `SQL fail-closed token ${token}`);
}
...
if (failures.length > 0) {
  console.error("A-16R-RUN-OFFICIAL-IMPORT check failed:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log("A-16R-RUN-OFFICIAL-IMPORT check passed.");
