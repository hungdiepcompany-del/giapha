...
} else if (!doc.includes("A16T_OWNER_APPLY_EVIDENCE_STATUS=MISSING")) {
...
for (const [content, token, label] of [
  ...
  [workLog, "A16T_APPLY_VERIFY_STATUS=BLOCKED_MISSING_OWNER_APPLY_VERIFY_EVIDENCE", "work log A-16T apply verify status"],
  [decisionLog, "Decision 236", "decision log A-16T apply verify entry"],
  [handoff, "A16T_APPLY_VERIFY_STATUS=BLOCKED_MISSING_OWNER_APPLY_VERIFY_EVIDENCE", "handoff A-16T apply verify status"],
]) {
