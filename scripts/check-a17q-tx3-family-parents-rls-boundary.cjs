...
if (normalizeTx3(tx3) !== normalizeTx2ToExpectedTx3(tx2)) {
  failures.push("0028 differs from 0027 beyond expected SECURITY DEFINER boundary and TX3 comment");
}
...
requireIncludes(tx3, "revoke execute on function public.execute_admin_a17q_legacy_family_reconciliation", "execute revoke preserved");
requireIncludes(tx3, "from public;", "PUBLIC execute revoked");
requireIncludes(tx3, "from anon;", "anon execute revoked");
requireIncludes(tx3, "grant execute on function public.execute_admin_a17q_legacy_family_reconciliation", "authenticated execute grant preserved");
