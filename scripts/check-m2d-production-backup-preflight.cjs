...
for (const token of ["workflow_dispatch", "expected_source_sha", "m2f-private-backup-release-control", "environment: backup-production", "--secrets-file", "trap 'rm -f \"$SECRETS_FILE\"' EXIT", "umask 077", "chmod 600 \"$SECRETS_FILE\"", "wrangler versions upload --env production", "BACKUP_SERVICE_INTERNAL_TOKEN: ${{ secrets.BACKUP_SERVICE_INTERNAL_TOKEN }}", "BACKUP_DATA_KEY_V1_B64: ${{ secrets.BACKUP_DATA_KEY_V1_B64 }}", "BACKUP_OBJECT_KEY_HMAC_V1_B64: ${{ secrets.BACKUP_OBJECT_KEY_HMAC_V1_B64 }}", "wrangler deploy --strict --env production --secrets-file", "wrangler versions deploy \"${TARGET_VERSION_ID}@100%\""]) requireText(workflow, token, `workflow ${token}`);
